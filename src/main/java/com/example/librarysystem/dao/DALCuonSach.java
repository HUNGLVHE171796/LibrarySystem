...
public class DALCuonSach extends ConnectDatabase implements I_DAL<CuonSach> {
    ...
    public DALCuonSach() throws SQLException {
        super();
    }
    ...
    public int getMaCuonSachByMaDauSach(int maDauSach) throws SQLException, ClassNotFoundException {
        openConnection();
        String sqlExec = "EXEC spLayMaCuonSachByMaDauSach ?";
        PreparedStatement statement = jdbcConnection.prepareStatement(sqlExec);
        statement.setEscapeProcessing(true);
        statement.setQueryTimeout(15);
        statement.setInt(1, maDauSach);
        ResultSet res = statement.executeQuery();
        int maCuonSach = 0;
        if (res.next()) {
            maCuonSach = res.getInt(1);
        }
        closeConnection();
        return maCuonSach;
    }
