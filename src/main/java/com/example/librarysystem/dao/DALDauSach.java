...
public class DALDauSach extends ConnectDatabase implements I_DAL<DauSach> {
	 ...
	 public DALDauSach() throws SQLException {
        super();
    }
...
//		closeConnection();
	...
	public List<DauSach> getAll() throws SQLException, ClassNotFoundException {
        ...
        try {
            // Tạo đối tượng DALDauSach
            DALDauSach dalDauSach = new DALDauSach();

            String loaiSach = "eBooks"; // Loại sách cần tìm
            int minRes = 0; // Tham số bắt đầu
            int maxRes = 10; // Tham số kết thúc (tối đa số bản ghi trả về)
            int maTheLoai = 2; // Mã thể loại (cần điều chỉnh theo cơ sở dữ liệu)
            String sort = "asc"; // Phương thức sắp xếp (asc hoặc desc)
            String search = "Sách Toán Học"; // Từ khóa tìm kiếm

            // Gọi phương thức getAllPhanTrang
            List<DauSach> dauSachs = dalDauSach.getAllPhanTrang(loaiSach, minRes, maxRes, maTheLoai, sort, search);

            // Kiểm tra xem danh sách có rỗng không và in ra các đối tượng DauSach
            if (dauSachs != null && !dauSachs.isEmpty()) {
                for (DauSach dauSach : dauSachs) {
                    System.out.println(dauSach); // In ra thông tin của đối tượng DauSach
                }
            } else {
