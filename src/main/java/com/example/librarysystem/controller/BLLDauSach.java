...
public class BLLDauSach extends HttpServlet {
	...
	private void dauSachNoiDung(HttpServletRequest request, HttpServletResponse response)
		...
		int code = Integer.parseInt(request.getParameter("maDauSach"));
             ...
             int maxCodeMuonTraSach = 0;
             int maxCodeMuonTraSachChiTiet = 0;

		try {
                     int maCuonSach = dal_cuonSach.getMaCuonSachByMaDauSach(code);
                     maxCodeMuonTraSach = dal_muonTraSach.maxCode("MuonTraSach");
                     maxCodeMuonTraSachChiTiet = dal_muonTraSachChiTiet.maxCode("muonTraSachChiTiet");
			DauSach dauSach = new DauSach();
			dauSach = dal_dauSach.GetOne(code);
                         request.setAttribute("maCuonSach", maCuonSach);
                          request.setAttribute("maxCodeMuonTraSach", maxCodeMuonTraSach);
                           request.setAttribute("maxCodeMuonTraSachChiTiet", maxCodeMuonTraSachChiTiet);
